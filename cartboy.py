...
class Store:
    ...
    def save(self):
        ...
        try:
            ...
            logging.critical("Unable to write data to savefile - {0}".format(
                savefile))
            logging.critical(
                "Please check the settings and make sure this file is in a writable location")
            sys.exit(1)
    ...
    def load(self):
        ...
        try:
            ...
            data = {
            }  # if we cannot load previous data, this is not critical, return empty dictionary
        self.data = data
    ...
    def increment_failcount(self, app_name):
        ...
        mainlog.debug(
            "Incrementing failcount for application '{0}', {1} -> {2}".format(
                app_name, current_failcount, new_failcount))
        self.set_failcount(app_name, new_failcount, False)

    def set_failcount(self, app_name, failcount, status=True):
        self.load()
        mainlog.debug("Setting failcount for application '{0}' to {1}".format(
            app_name, failcount))
        try:
            self.data[app_name] = {'failcount': failcount,
                                   'status': status,
                                   'time': datetime.datetime.now()}
            self.save()
        except:
            mainlog.warning(
                "Unable to set failcount for application '{0}', savefile corrupt?".format(
                    app_name))

    def reset_failcount(self, app_name):
        self.load()
        mainlog.debug("Resetting failcount for application '{0}'".format(
            app_name))
    ...
    def get_failcount(self, app_name):
        self.load()
        mainlog.debug("Getting failcount for application '{0}'".format(
            app_name))
        if app_name in self.data:
            try:
                failcount = int(
                    self.data[app_name]['failcount']
                )  # if for whatever reason failcount is not an integer, the script goes boom
            except:
                mainlog.warning(
                    "Unable to get the failcount for application '{0}', the store appears to be corrupt.".format(
                        app_name))
                failcount = 0
        else:
            failcount = 0
    ...
    def get_last_time(self, app_name):
        ...
        if app_name in self.data:
            try:
                ...
                mainlog.warning(
                    "Unable to get last time for application '{0}'".format(
                        app_name))
        return last_time
    ...
    def get_last_status(self, app_name):
        ...
        if app_name in self.data:
            try:
                ...
                mainlog.warning(
                    "Unable to get last status for application '{0}'".format(
                        app_name))
        return last_status
...
def get_pid(app):
    ...
    if not len(result) == 1 or not len(result[0].split()) == 1:
        mainlog.debug(
            "Execution of '{0}' returned an unexpected result".format(cmd))
        return False
    try:
        pid = int(result[0])
        return pid
...
def valid_app(app_path):
    ...
    if not os.path.exists("{0}/pid".format(app_path)):
        mainlog.warning(
            "Application: '{0}' is missing its 'pid' script".format(app_path))
        return False
    if not os.path.exists("{0}/start".format(app_path)):
        mainlog.warning(
            "Application: '{0}' is missing its 'start' script".format(
                app_path))
        return False
    return True
...
def start_application(app_path):
    ...
    if failcount >= max_start_fails:
        ...
        if not last_time:
            die("Unable to get last time for application '{0}', corrupted history file?".format(
                app_name))
        ...
        if not backoff_time_elapsed.seconds > local_backoff_time:
            ...
            mainlog.warning(
                "Starting of application '{0}' has failed {1} times, skipping.".format(
                    app_name, failcount))
            mainlog.warning(
                "The next attempted startup of this application will be made after {0} seconds".format(
                    wait_time_till_next_start))
            print "last status:", last_status
            print "last time:", last_time
            if os.path.exists("{0}/alert".format(app_path)):
                mainlog.debug(
                    "Executing the alert for application '{0}'".format(
                        app_name))
                print execute("sh {0}/alert".format(app_path))
            return False
    mainlog.debug("Exec output: {0}".format(execute("sh {0}/start".format(
        app_path))))
    time.sleep(1)
    if get_pid(app_path):
...
def main():
    ...
    if not apps:
        mainlog.debug(
            "No applications configured, please see the man page if you need help configuring Cartboy")
        return False
    ...
    for app_path in apps:
        ...
        if not pid:
            mainlog.debug("No pid found for {0}, starting it up".format(
                app_name))
            start_application(app_path)
            continue
        if is_running(pid):
            mainlog.debug(
                "Application appears to be running (pid: {0})".format(pid))
        else:
...
if __name__ == "__main__":
    ...
    os.unlink(
        lock_file_name
    )  # FIXME: for some reason lock file is not removed if there is a "start" of some application
